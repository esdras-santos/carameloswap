...
contract CarameloSwapExchange {
    ...
    function addLiquidity(uint256 minLiquidity, uint256 maxTokens, uint256 deadline) public payable returns(uint256){
        ...
        if (totalLiquidity > 0) {
            ...
            require(Factory(factory).getExchange(token) == address(this));
    ...
    function getInputPrice(uint256 inputAmount, uint256 inputReserve, uint256 outputReserve) private returns(uint256) {
    ...
    function getOutputPrice(uint256 outputAmount, uint256 inputReserve, uint256 outputReserve) private returns(uint256){
        require(inputReserve > 0 && outputReserve > 0);
    ...
    function tokenToTokenSwapInput(uint256 tokensSold, uint256 minTokensBought, uint256 minEthBought, uint256 deadline, address tokenAddr) public returns(uint256) {
        address exchangeAddr = Factory(factory).getExchange(tokenAddr);
    ...
    function tokenToTokenOutput(uint256 tokensBought, uint256 maxTokensSold, uint256 maxEthSold, uint256 deadline, address buyer, address recipient, address exchangeAddr) private returns(uint256) {
        ...
        require(IERC20(token).transferFrom(buyer, address(this), tokensSold));
        uint256 ethSold = Exchange(exchangeAddr).ethToTokenTransferOutput{value: ethBought}(tokensBought, deadline, recipient);
        emit EthPurchase(buyer, tokensSold, ethBought);
        return tokensSold;
    }
    ...
    function getEthToTokenInputPrice(uint256 ethSold) public returns(uint256) {
        require(ethSold > 0);
        uint256 tokenReserve = IERC20(token).balanceOf(address(this));
        return getInputPrice(ethSold, address(this).balance, tokenReserve);
    }

    function getEthToTokenOutputPrice(uint256 tokensBought) public returns(uint256){
    ...
    function getTokenToEthInputPrice(uint256 tokensSold) public returns(uint256){
    ...
    function getTokenToEthOutputPrice(uint256 ethBought) public returns(uint256) {
        require(ethBought > 0);
        uint256 tokenReserve = IERC20(token).balanceOf(address(this));
    ...
    function transferFrom(address from, address to, uint256 value) public returns(bool){
        require(allowances[owner][spender] >= value);
        balances[from] -= value;
        balances[to] += value;
